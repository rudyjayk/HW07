	...
	void update(int clock) 
	{
		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				int index;
				index = clock - plane->arrival_time;

				//update total_wait and num_served for the landing queue
				landing_queue->total_wait += index;
				...
				*/
				...
				//						This algorithm is used so a number between the min and max service time is chosen nothing else

				//add the plane to the service queuethe
